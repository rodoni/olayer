...
use olayer_core::geodesy::conversions::{ecef_to_lla, lla_to_ecef};
use olayer_core::geodesy::{Ellipsoid, GeodeticSolver, HaversineSolver, LatLon, VincentySolver};
...
fn benchmark_solvers(c: &mut Criterion) {
    ...
    c.bench_function("vincenty_inverse_near_antipodal", |b| {
        b.iter(|| {
            vincenty.inverse(
                black_box(&near_antipodal_1),
                black_box(&near_antipodal_2),
                black_box(&ellipsoid),
            )
        })
    });
