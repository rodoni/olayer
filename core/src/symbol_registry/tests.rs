...
fn test_scale_filtered_symbol_resolution() {
    ...
    let style = StyleRegistry {
        layers: std::collections::HashMap::from([(
            "sym".to_string(),
            vec![RuleStyle {
                name: "far".to_string(),
                min_scale: Some(100.0),
                max_scale: None,
                stroke: None,
                fill: Some(FillStyle {
                    color: "#ff0000".to_string(),
    ...
    let near = registry
        .resolve_symbol_at_scale("sym", &style, 10.0)
        .unwrap();
    let far = registry
        .resolve_symbol_at_scale("sym", &style, 200.0)
        .unwrap();
    assert!(matches!(
        near.primitives[0],
        SymbolPrimitive::Circle { fill: None, .. }
    ));
    assert!(matches!(
        far.primitives[0],
        SymbolPrimitive::Circle { fill: Some(_), .. }
    ));
}
