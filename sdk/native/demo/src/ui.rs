...
pub fn draw_flight_profile(
    ...
    for dist in (-30000..=50000).step_by(2000) {
        ...
        let lon_offset =
            (dist as f64 * target.heading_rad.sin()) / (r_earth * target.position.lat.cos());
        route_coords.push(LatLon::new(
            target.position.lat + lat_offset,
            target.position.lon + lon_offset,
    ...
    if let Ok(profile) = controller.terrain.get_vertical_profile(&route_coords, step) {
        ...
        let max_elev = profile
            .iter()
            .map(|pt| pt.ground_elevation)
            .fold(1000.0f64, |m, val| m.max(val))
            .max(target.position.height)
            * 1.2;
        ...
        for pt in &profile {
            points.push(egui::pos2(
                get_x(pt.distance_meters),
                get_y(pt.ground_elevation),
            ));
        }
        ...
        let ground_under_ac = controller
            .terrain
            .get_elevation(
                target.position.lat.to_degrees(),
                target.position.lon.to_degrees(),
            )
            .unwrap_or(0.0);
        let clearance = target.position.height - ground_under_ac;
        let hazard = clearance < 300.0;
        let clearance_color = if hazard {
